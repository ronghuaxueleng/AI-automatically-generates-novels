...
def init_api():
    try:
        ...
        logger.info(f"Available models: {[model.name for model in models.data]}")
        return client
    except Exception as e:
...
def generate_content(prompt, stream=True):
    try:
        ...
        if not stream:
            response_time = (time.time() - start_time) * 1000
            token_stats.record_call(success=True, response_time=response_time, tokens_used=response.usage.input_tokens + response.usage.output_tokens)
